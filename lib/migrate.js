...
function reverseSequelizeColType(col, prefix = 'Sequelize.') {
  ...
  // Sequelize.CHAR
  if (attrObj instanceof Sequelize.CHAR || attrObj.toString() === 'CHAR') {
  ...
  // Sequelize.STRING
  if (attrObj instanceof Sequelize.STRING || attrObj.toString() === 'STRING' || attrObj.toString().includes('VARCHAR')) {
  ...
  // Sequelize.TEXT
  if (attrObj instanceof Sequelize.TEXT || attrObj.toString() === 'TEXT') {
  ...
  // Sequelize.DOUBLE
  if (attrObj instanceof Sequelize.DOUBLE || attrObj.toString() === 'DOUBLE') {
  ...
  if (attrObj instanceof Sequelize.NUMBER || attrObj.toString() === 'INTEGER' || attrObj.toString() === 'BIGINT' || attrObj.toString() === 'FLOAT') {
    let ret = attrObj.key;
    if (options.length) {
      ret += `(${options.length}`;
  ...
  if (attrObj instanceof Sequelize.ARRAY || attrObj.toString() === 'ARRAY') { return `${prefix}ARRAY(${reverseSequelizeColType(attrObj, prefix)})`; }

  // Sequelize.ENUM
  if (attrObj instanceof Sequelize.ENUM || attrObj.toString() === 'ENUM') { return `${prefix}ENUM('${options.values.join("', '")}')`; }

  // Simple types
  if (attrObj instanceof Sequelize.BOOLEAN || attrObj.toString() === 'BOOLEAN' || attrObj.toString() === 'TINYINT(1)') { return `${prefix}BOOLEAN`; }

  if (attrObj instanceof Sequelize.TIME || attrObj.toString() === 'TIME') { return `${prefix}TIME`; }

  if (attrObj instanceof Sequelize.DATEONLY || attrObj.toString() === 'DATEONLY') { return `${prefix}DATEONLY`; }

  if (attrObj instanceof Sequelize.DATE || attrObj.toString() === 'DATE' || attrObj.toString() === 'DATETIME') { return `${prefix}DATE`; }

  // Not documented, really?
  if (attrObj instanceof Sequelize.HSTORE || attrObj.toString() === 'HSTORE') { return `${prefix}HSTORE`; }

  if (attrObj instanceof Sequelize.JSONB || attrObj.toString() === 'JSONB') { return `${prefix}JSONB`; }

  if (attrObj instanceof Sequelize.JSON || attrObj.toString() === 'JSON') { return `${prefix}JSON`; }

  if (attrObj instanceof Sequelize.UUID || attrObj.toString() === 'UUID') { return `${prefix}UUID`; }

  // Virtual data type, we must to skip it
  if (attrObj instanceof Sequelize.VIRTUAL || attrObj.toString() === 'VIRTUAL') { return `${prefix}VIRTUAL`; }
